...
void wstrcpy(unsigned short *wchar, unsigned char *bchar)
{
    while(*bchar != NUL)
    {
        *wchar++ = *bchar++;
...
char *strcvt(unsigned short *wchar)
{
...
char *InsertRecord(SQLCHAR *charname, SQLCHAR *chartype, int *sendbuflen)
{
   ...
   *sendbuflen = 0;
   ...
   //_mbscpy(datasrcname, (const unsigned char *)"Northwind");
    wstrcpy((unsigned short *)datasrcname, (unsigned char *)"CharacterDNS");
   ...
   wstrcpy((unsigned short*)sqlstatement, (unsigned char *)"INSERT INTO character(name, type) VALUES (?, ?)");
...
char *ReadRecords(int *sendbuflen)
{
   ...
   *sendbuflen = 0;
   ...
   wstrcpy((unsigned short*)sqlstatement, (unsigned char *)"SELECT name,type FROM character");
   //ret = SQLPrepare(hstmt1, sqlstatement, SQL_NTS);
   //ret = SQLExecute(hstmt1);
